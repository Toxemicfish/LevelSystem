...
public class playerLevelYML {
    ...
    public static void reloadLevelFile() {
        if (levelFile == null) {
            levelFile = new File(Bukkit.getPluginManager().getPlugin("Survival").getDataFolder(), "playerLevels.yml");
        }
        level = YamlConfiguration.loadConfiguration(levelFile);

        InputStream defConfigStream = Bukkit.getPluginManager().getPlugin("Survival").getResource("playerLevels.yml");
